...
export class PayOsService {
  ...
  private extractPaymentResponse(data: any): {
    payUrl: string;
    payOsOrderId: string;
  } {
  ...
  async createPayment(
    amount: number,
    orderCode: number | string,
  ): Promise<{ payUrl: string; payOsOrderId: string }> {
    const { clientId, apiKey, checksumKey, endpoint } = getPayOsConfig();

    if (!checksumKey || !clientId || !apiKey) {
      throw new InternalServerErrorException(
        'PayOs configuration is missing required fields',
      );
    }
    ...
    const responseData = await this.sendPaymentRequest(
      endpoint,
      paymentBody,
      signature,
      headers,
  ...
  private createVerificationHeaders(
    clientId: string,
    apiKey: string,
  ): Record<string, string> {
    return {
  ...
  async verifyPayment(payOsOrderId: string): Promise<boolean> {
    ...
    const data = await this.sendVerificationRequest(
      endpoint,
      payOsOrderId,
      headers,
    );
